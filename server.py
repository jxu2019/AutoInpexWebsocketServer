...
def message_received(client, server, message):
    try:
        ...
        if len(data["vinCode"]) > 0 and data["autoInspexID"] == AutoInspexID:
            ...
            retData ={"vinCode": data["vinCode"] ,"vehicleId": data["vehicleId"] ,"autoInspexID": data["autoInspexID"] ,"uuid": data["uuid"], "sequenceNo":CameraPosition+"", "inspexIQConnectionId": data["inspexIQConnectionId"], "image_url":url};
            retJson= json.dumps(retData);
            writeLog(retJson);
